...
def _get_using_tools_section() -> str:
    return (
        ...
        "Note: This runs on macOS. Do NOT use GNU-only commands like `timeout` "
        "(use `perl -e 'alarm shift; exec @ARGV' -- N cmd` instead). "
        "Avoid `pkill -f` with broad patterns — it can hang. Use `kill PID` if you know the PID.\n"
