...
def _get_team_section() -> str:
    return (
        ...
        "- **spawn_teammate**(name, role, prompt): Start a teammate agent to work on a task\n"
        "  - This starts an agent thread that will read inbox and respond\n"
        "  - Example: spawn_teammate(name='pm', role='product_manager', prompt='Review requirements for...')\n"
        "- **send_message**(to, content, msg_type): Send a message to a teammate's inbox\n"
        "  - Note: This only sends to inbox, does NOT start agent. Use spawn_teammate for active work.\n"
        "  - Example: send_message(to='pm', content='Please review the requirements')\n"
        "- **read_inbox**: Read messages from your inbox (auto-drains after reading)\n"
        "- **list_teammates**: List all teammates with their roles and status\n"
        "- **broadcast**(content): Send a message to all teammates\n\n"
        "## How to use team\n"
        "1. To get a teammate to DO something: use spawn_teammate (starts agent thread)\n"
        "2. To send a quick message: use send_message (writes to inbox only)\n"
        "3. To check for replies: use read_inbox\n"
        "4. Team members communicate via file-based inbox (.team/inbox/*.jsonl)\n"
        "5. Do NOT manually read/write inbox files - use the team tools instead\n\n"
        "## Common patterns\n"
        "- User: 'Have pm review the requirements' → spawn_teammate(name='pm', role='product_manager', prompt='Review...')\n"
        "- User: 'Ask pm a question' → send_message(to='pm', content='...') then read_inbox() later\n"
        "- User: 'Have coder implement the feature' → spawn_teammate(name='coder', role='coder', prompt='...')\n"
        "- User: 'Check for replies' → read_inbox()\n"
        "- User: 'Tell everyone about the deadline' → broadcast(content='...')\n\n"
        "## Important\n"
        "- spawn_teammate starts an agent that will process its inbox and respond\n"
        "- send_message only writes to inbox, does NOT guarantee a response\n"
        "- After spawning, check read_inbox() periodically for replies"
    )
